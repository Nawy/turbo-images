...
import {INVALID_INPUT, EMPTY_INPUT, InputTextForm} from "../models/input-text.model";
  ...
  onSignup() {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      if (!this.nameRegExp.test(this.signupData.name)) {
        this.nameForm.setValue(INVALID_INPUT, "Name is wrong!")
        return reject();
      }
      //correct email
      if(this.signupData.email != "") {
        if (!this.emailRegExp.test(this.signupData.email)) {
          this.emailForm.setValue(INVALID_INPUT, "Email is wrong!")
          return reject();
        }
      }
      //correct password
      if(!this.passwordRegExp.test(this.signupData.password)) {
        this.passwordForm.setValue(INVALID_INPUT, "Password is wrong!")
        return reject();
      }

      //check similarity of passwords
      if(this.signupData.password != this.signupData.passwordRepeat) {
        this.repeatPasswordForm.setValue(INVALID_INPUT, "Password doesn't match!")
        return reject();
      }
      return resolve()
    }).then(() => {
      ...
      if(res != false) {
        this.nameForm.setValue(INVALID_INPUT, `${this.signupData.name} already exists!`);
        return Promise.reject(null)
      }
      return this.authorizedService.isExistsNameOrEmail(this.signupData.email);
    }).then(res => {
      if(res != false) {
        this.emailForm.setValue(INVALID_INPUT, `${this.signupData.email} already exists!`);
        return Promise.reject(null)
      }
